...
(function (root, factory) {
  ...
  function matchListLine(line) {
    ...
    return { indent: m[1].length, ordered: ordered, startNum: startNum, text: m[3] };
  }

  // 递归解析：同缩进 → 兄弟；更深缩进 → 子列表
  function parseList(lines, i, S, t, opts) {
    const first = matchListLine(lines[i]);
    const baseIndent = first.indent;
    const ordered = first.ordered;
    let startNum = first.startNum;
    let items = [];
    let cur = null;
    let j = i;
    ...
    while (j < lines.length) {
      ...
      if (m.indent === baseIndent) {
        if (m.ordered !== ordered) break; // 类型切换 → 结束当前列表
        cur = { text: m.text, task: null, child: '' };
        // 任务项 - [ ] / - [x]
        const tm = m.text.match(/^\[([ xX])\]\s+(.*)$/);
        if (tm) { cur.task = tm[1].toLowerCase() === 'x'; cur.text = tm[2]; }
        items.push(cur);
        j++;
      } else {
        // 更深缩进：若仍是列表行 → 子列表；否则视为上一项续行（缩进文本）
        if (!cur) { j++; continue; }
        const r = parseList(lines, j, S, t, opts);
        if (r && r.count > 0) {
          cur.child = r.html;
          j = r.next;
        } else {
    ...
    if (items.length === 0) return { html: '', next: j, count: 0 };
    const hasTask = items.some(function (x) { return x.task !== null; });
    const tag = ordered ? 'ol' : 'ul';
    const contStyle = ordered
      ? S.ol.replace(/color:[^;]+;/, '')
      : S.ul.replace(/color:[^;]+;/g, '');
    const listStyle = hasTask ? 'list-style:none;padding-left:16px;' : '';
    let html = '<' + tag + ' style="' + (hasTask ? S.ul.replace('padding-left:24px;', 'padding-left:14px;') : (ordered ? S.ol : S.ul)) + '"' + (ordered && startNum !== 1 ? ' start="' + startNum + '"' : '') + '>';
    items.forEach(function (it) {
      let liStyle = hasTask ? 'list-style:none;' + S.li : S.li;
      let prefix = '';
      if (it.task !== null) {
        const sym = it.task ? '✅' : '⬜';
        prefix = '<span>' + sym + '</span> ';
      }
      const textHtml = inline(it.text, S, t, opts);
      html += '<li style="' + liStyle + '">' + prefix + textHtml + it.child + '</li>';
    });
    html += '</' + tag + '>';
    return { html: html, next: j, count: items.length };
  }

  /* ==================== 媒体占位 / 段落 ==================== */

  function isMediaPlaceholder(line) {
    const m = line.match(/^\[(视频|音频)：(.*)\]$/);
    return m ? { type: m[1], title: m[2] } : null;
  }
  ...
  function render(md, opts) {
    ...
    while (i < lines.length) {
      ...
      if (hm) {
        ...
        if (lv === 1) { i++; continue; } // H1 由公众号标题承担
        if (lv === 2) {
      ...
      if (/^```/.test(line)) {
        ...
        const dots = S.preDots
          ? '<span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:' + t.dotRed + ';margin-right:6px;"></span><span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:' + t.dotYellow + ';margin-right:6px;"></span><span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:' + t.dotGreen + ';"></span>'
          : '';
