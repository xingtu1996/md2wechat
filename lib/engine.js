...
(function (root, factory) {
  ...
  const THEMES = {
    ...
    'reviewBlack': { // 评测风（黑）· 黑卡直角变体（新增主题，不改评测风）
      name: '评测风（黑）', group: '观点随笔', main: '#d71a1b',
      h2: 'lineAccent', h3: 'line', quote: 'blockBlack', pre: 'dark', dot: '#d71a1b',
      desc: '评测风版式 + 引用/摘要块为纯黑直角卡（#1a1a1a 米白字），高级感无彩色底'
    },
    'ink': { // 墨韵留白（人文成长）
  ...
  function stylesOf(t) {
    ...
    const quoteStyle = {
      ...
      blockBlack: `margin:26px 0;padding:20px 22px;background:#1a1a1a;border-radius:14px;color:#f5f5f5;font-size:15px;line-height:1.8;letter-spacing:0.5px;`,
